...
if (trim($full_name) == "") {
    ...
    if ($row) {
        ...
        $sql = "INSERT INTO users (full_name, username, password, role, active)
				VALUES ('$full_name', '$username', '$password_md5', 0, 1)";
        $result = $connect->query($sql);
        ...
        if (!$result) {
            ...
            $_SESSION['auth_message'] = 'Đăng ký thành công !';
            $_SESSION['full_name_old'] = '';
            $_SESSION['username_old'] = '';
            $_SESSION['password_old'] = '';
            header("Location: index.php?auth=pages/auth/regis");
            exit();
